...
public class HttpClientUtil {
            ...
            if (type == 0) {
                ...
                for (String key : fileMap.keySet()) {
                    multiEntity.addPart(key,
                        new ByteArrayBody(getUrlFileBytes(fileMap.get(key)), key));
                }
            }
            // 加入相关参数 默认编码为utf-8  
     ...
     * @return
     */
    public static String httpFileRequest(String url, MultipartEntity multipartEntity,
                                         HttpHost proxy) {
        String responseStr = null;
        // 判断输入的值是是否为空  
        if (null == url || "".equals(url)) {
    ...
    public static void main(String[] args) throws URISyntaxException, ClientProtocolException,
        ...
        String url = "https://api.smzdm.com/v1/youhui/articles/?limit=50&article_date=&F=ANDROID&S=Z3XYNAAFDPYHWKO9RWPYJIJQ6FP4JY8B&V=335&WEIXIN=1%20HTTP/1.1";

        String str = HttpClientUtil.asynGet(url, null);
